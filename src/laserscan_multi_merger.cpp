...
void LaserscanMerger::laserscan_topic_parser()
{
    ...
    for(int i=0; i<tokens.size(); ++i)
    {
        ...
        if(!found) {
            ROS_ERROR_STREAM("Topic not in topic list or not of type sensor_msgs/LaserScan:" << tokens[i]);
            error = true;
        }
    }
...
void LaserscanMerger::scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan, std::string topic)
{
    ...
    tfListener_.waitForTransform(scan->header.frame_id.c_str(), destination_frame.c_str(), scan->header.stamp, ros::Duration(20.0));
